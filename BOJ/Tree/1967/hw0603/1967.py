...
# 임의의 한 노드(루트 노드, N1)에서 가장 멀리 있는 노드(N2)를 찾음
dist = [-1] * (n + 1)  # 거리 배열 초기화
dist[1] = 0  # 현재 노드 자신과의 거리는 0
dfs(1)
N2 = dist.index(max(dist))

# 위에서 찾은 노드(N2)에서 가장 멀리 있는 노드(N3)를 찾음
dist = [-1] * (n + 1)
dist[N2] = 0
dfs(N2)
N3 = dist.index(max(dist))

print(dist[N3])  # N2에서 가장 멀리 있는 노드 N3에 대하여, N2와 N3사이의 거리를 출력
